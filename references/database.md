...
Execute read-only SQL statements directly using `spelunk query "<SQL>"`:
...
spelunk query "SELECT file_path FROM file_imports WHERE imported_path = 'src/services/db.ts';"
...
spelunk query "SELECT path FROM files_fts WHERE files_fts MATCH 'Spelunk';"
```
