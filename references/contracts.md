...
Defined in [status.ts](../src/commands/status.ts). Returns workspace index status and cache freshness.
...
Defined in [query.ts](../src/commands/query.ts). Executes custom read-only SQL queries against `.spelunk/data.db`.
